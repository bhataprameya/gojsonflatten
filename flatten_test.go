...
func TestFlattenNoArray(t *testing.T) {
	cases := []struct {
		test   string
		want   map[string]interface{}
		prefix string
	...
	for i, test := range cases {
		t.Run(fmt.Sprintf("test: %v", i), func(t *testing.T) {
			...
			err := json.Unmarshal([]byte(test.test), &m)
			assert.NoError(t, err)
			got, err := FlattenNoArray(m.(map[string]interface{}), test.prefix, test.style, test.depth)
			assert.NoError(t, err)
			deepEquals(t, i, got, test.want)
		})
	}
...
func TestFlattenStringNoArray(t *testing.T) {
	cases := []struct {
		test   string
		want   string
		prefix string
	...
	for i, test := range cases {
		t.Run(fmt.Sprintf("test: %v", i), func(t *testing.T) {
			got, err := FlattenStringNoArray(test.test, test.prefix, test.style, test.depth)
			errors.Is(err, test.err)
			if err == nil {
				assert.Equal(t, got, strings.Map(nixws, test.want))
			}
...
func TestFlattenNoArrayWithVaryingDepth(t *testing.T) {
	cases := []struct {
		test   string
		want   map[string]interface{}
		prefix string
		...
		// Test data for depth 2 with array and prefix
		{
			...
			map[string]interface{}{
				"aa_foo.jim":  "bean",
				"aa_fee":      "bar",
				"aa_n1.alist": []interface{}{"a", "b", "c", map[string]interface{}{"d": "other", "e": "another"}},
				"aa_number":   1.4567,
				"aa_bool":     true,
			},
			// Prefix, SeparatorStyle, and depth
